...
documentation**, meaning technical reports, official release notes, and model
cards written by the org that trained the *parent*. Every label ships with a citation. The
...
*cited, org-documented* label. Contributions of new pairs are welcome via pull
request to the
[manifest](https://github.com/AwaisAdilKhokhar/modelDNA/blob/main/benchmarks/lineagebench_pairs.json),
provided each pair carries a citation to the publishing org's own
documentation, never to a `base_model` tag.
...
benchmark regenerates offline, with no Hub round-trip and no gated licenses:
