...
The design is shaped by one asymmetry: the worst failure mode of a lineage tool is not a missed detection but a false accusation. Every decision downstream of that — conservative thresholds, an abstention band, a zero-false-positive ship gate, background distributions printed next to every score — trades recall for precision on purpose.

This report makes four contributions:

1. **A fingerprint design** that captures four published signal families from ~100–300 MB of byte-range reads per 7B model, with sample positions that are pure functions of (seed, canonical role, layer, tensor size) — the property that makes fingerprints of different models directly comparable, and that later turns out to enable merge decomposition (Section 4).
...
Relative to all of the above, this report's claims are deliberately modest on signal novelty — the four families are from the literature — and concrete on three things the literature does not provide: the composition (calibration, abstention, background reporting, one command), the sampling design that makes fingerprints of different models element-aligned, and the demonstration that this alignment is sufficient to decompose merges.
...
A fingerprint serializes to roughly 1.5 MB gzipped. The reference database — 37 foundation bases spanning Llama, Qwen, Mistral, Gemma, Phi, DeepSeek, Yi, Falcon, OLMo and others, plus deliberate hard negatives such as OpenLLaMA (Llama shapes, independent training) — is a 57 MB archive that installs in seconds, so a scan only ever fetches the suspect. No weights are stored or redistributed at any point, which also keeps the database license-clean.
...
- **Missing features are imputed with unrelated-pair background means**, so a signal that could not be computed (PCS skipped because shapes differ, say) pushes the score toward the background — absence of a signal is never evidence in either direction.
...
The claimed-versus-detected consistency flag — the feature the opening example of the README shows — fires only when the repo makes an explicit lineage claim that the weight evidence contradicts at high confidence. A missing `base_model` tag is never "inconsistent"; it is just missing.
...
Every mainstream mergekit method produces, per tensor, a weight matrix that is a linear or near-linear combination of the parents' corresponding tensors. Linear interpolation and task arithmetic are linear by definition — note that task arithmetic, W = W_base + Σᵢ λᵢ(Wᵢ − W_base), is exactly a sum-to-one linear combination once the base is included as a term with weight 1 − Σλᵢ. Slerp is not linear in general, but merge parents are fine-tunes of a shared base with pairwise cosines of 0.99+; at those angles slerp coincides with linear interpolation up to a correction quadratic in the (tiny) angle. TIES and DARE apply elementwise sign-election and random sparsification to the task vectors, which breaks exact linearity per element but preserves it in expectation — the deviation appears as fit residual rather than as bias in the weights, a point Section 7.3 confirms empirically.
...
The two limitation cases behaved exactly as documented. SOLAR-10.7B, a depth-upscale from 32 to 48 layers, produced an honest `NO_MATCH` with the layer-count mismatch reported — the tool does not currently attempt depth-alignment search, and says so rather than guessing. The GPTQ copy abstained on the packed int4 tensors while still ranking the true parent first on the signals it could compute.

Two caveats, reported rather than hidden. First, AlphaMonarch-7B — the merge-chain case — resolved to the correct family as `SAME_FAMILY_UNRESOLVED` rather than `LIKELY_MERGE`: all its ancestors are Mistral fine-tunes, so no cross-family split exists for the scan heuristic to notice, and the scan-level merge flag is family-based in v0.1. (The decomposition tool, given candidates, resolves this model completely — Section 7.3.) Second, Nous-Hermes-llama-2-7b was attributed to the correct parent but classed `QUANTIZED_COPY` rather than `FINE_TUNE`: its fine-tune moved the weights so little that its PCS cosine against Llama-2-7b (0.9998) crosses the dtype-recast rule's 0.999 threshold, and because the repos ship in different dtypes (bfloat16 versus the parent's float16) the recast rule fires before the fine-tune rule. The subclass boundary between "recast of the same values" and "very light fine-tune" needs a delta-based discriminator rather than a cosine threshold. Top-line metrics are unaffected — the parent, probability, and ranking are right — but the subclass boundary needs work and I would rather record that here than have a reader discover it.

A scoping note: at 13 positives and 107 negatives this benchmark is deliberately smaller than the ≥300-pair target the project's own planning document sets. It is honestly scoped — every ground-truth label carries a citation to org documentation — but the AUROC of 1.0 should be read as "no errors at this scale", not as a claim that errors are impossible. Growing the benchmark is mechanical (the fingerprint pipeline is the expensive part, and it is now fast) and is the highest-priority follow-up.
...
**Scale.** Fifteen benchmark suspects, 55 models in the public graph, 37 indexed bases. Nothing in the method caps this — fingerprinting is ~2 minutes per model and the pipeline is automated — but the numbers in this report are the numbers, and they come with the error bars of their size.

## 9. Responsible use

The tool's framing assumes its outputs will end up in disputes. Three rules are enforced in the product, not just the documentation. Verdicts are phrased as statistical consistency and carry their background distributions; the consistency flag fires only against explicit claims, never against silence; and thresholds are set so that the shipped configuration has produced zero false positives across both benchmark suites — with the abstention classes absorbing every case where the evidence is real but insufficient. Reproducibility is the other half of accountability: any verdict can be re-derived by anyone with `modeldna compare a b --report`, a pinned database version, and no privileged access — the property whose absence made the Pangu dispute unresolvable.

## 10. Artifacts

Everything in this report is public:

- **Tool**: `pip install modeldna` — https://github.com/AwaisAdilKhokhar/modelDNA (Apache-2.0)
- **Live scanner**: https://huggingface.co/spaces/AwaisAdilKhokhar/modelDNA — paste a repo id, verdict in ~2 minutes
...
- **Benchmarks**: `benchmarks/` in the repository — ground-truth manifests with per-pair citations, committed fingerprint caches, and harnesses that reproduce every number here offline (`--no-fetch`)
